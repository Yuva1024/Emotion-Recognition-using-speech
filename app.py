...
def load_model():
    ...
    try:
        ...
        # Check which model type we loaded
        model_input_shape = model.input_shape
        if len(model_input_shape) == 3:  # CNN model (None, 39, 130)
            model_type = "CNN"
        else:  # Simple model (None, 5070)
            model_type = "Simple"
            
        st.success(f"✅ {model_type} model loaded successfully from {model_path}")
        st.info(f"📊 Model input shape: {model_input_shape}")
...
def predict_emotion(model, label_encoder, features, model_type):
    ...
    try:
        ...
        # Normalize features per sample (same as training)
        features_normalized = (features - np.mean(features)) / (np.std(features) + 1e-8)
        
        # Reshape for model input based on model type
        if model_type == "Simple":
            # Simple model expects flattened input
            # Original shape: (39, 130) -> Flattened shape: (1, 39*130) = (1, 5070)
            features_reshaped = features_normalized.reshape(1, -1)
        else:
            # CNN model expects 3D input
            # Original shape: (39, 130) -> CNN shape: (1, 39, 130)
    ...
    with st.sidebar:
        ...
        if label_encoder is not None:
            ...
            <div class="info-card">
                ...
                <p><strong>Best performing:</strong> Happy (83.8%)</p>
                <p><strong>Good performance:</strong> Angry (78.4%), Calm (78.4%)</p>
                <p><strong>Needs improvement:</strong> Sad (40.5%)</p>
                <p><strong>Overall accuracy:</strong> 67%</p>
            </div>
            """, unsafe_allow_html=True)
